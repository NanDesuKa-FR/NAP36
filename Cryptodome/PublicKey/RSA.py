...
# Embedded file name: lib\site-packages\Cryptodome\PublicKey\RSA.py
__all__ = [
 'generate', 'construct', 'import_key',
 'RsaKey', 'oid']
...
class RsaKey(object):
    ...
    def __eq__(self, other):
        if self.has_private() != other.has_private():
            ...
            if not self.has_private():
                return True
            return self.d == other.d and self.q == other.q and self.p == other.p and self.u == other.u
        ...
        if passphrase is not None:
            ...
            if randfunc is None:
                ...
                if format == 'OpenSSH':
                    ...
                    if bord(e_bytes[0]) & 128:
                        e_bytes = b'\x00' + e_bytes
                    if bord(n_bytes[0]) & 128:
                        n_bytes = b'\x00' + n_bytes
                    keyparts = [
                     b'ssh-rsa', e_bytes, n_bytes]
                    ...
                    if self.has_private():
                        ...
                        key_type = 'PUBLIC KEY'
                        binary_key = _create_subject_public_key_info(oid, DerSequence([self.n,
                         self.e]))
                if format == 'DER':
                    return binary_key
            if format == 'PEM':
