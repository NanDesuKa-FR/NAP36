...
# Embedded file name: lib\site-packages\Cryptodome\Cipher\_mode_ctr.py
"""
Counter (CTR) mode.
"""
__all__ = [
...
class CtrMode(object):
    ...
    def __init__(self, block_cipher, initial_counter_block, prefix_len, counter_len, little_endian):
        ...
        if result:
            raise ValueError('Error %X while instatiating the CTR mode' % result)
...
def _create_ctr_cipher(factory, **kwargs):
    ...
    if counter is not None:
        if (nonce, initial_value) != (None, None):
    ...
    if counter is None:
        ...
        if len(initial_counter_block) != factory.block_size:
            raise ValueError('Size of the counter block (%d bytes) must match block size (%d)' % (
