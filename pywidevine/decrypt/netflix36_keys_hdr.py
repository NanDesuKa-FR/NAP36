...
# Embedded file name: Nap36Src\pywidevine\decrypt\netflix36_keys_hdr.py
if __name__ == 'pywidevine.decrypt.netflix36_keys_hdr':
    ...
    def GettingKEYS_Netflix(nfID):
        ...
        global rsa_key_bin
        rsa_key_bin = 'rsa_keys_hdr.bin'
        msl_data_file = 'msl_data_keys_hdr.json'
        manifest_file = 'manifest_keys_hdr.json'
        ...
        print('\nGetting HDR-DV Keys...')
        KEYS = []
        manifest_keys_dv = str(MSL.load_manifest(MSL(int(nfID)), int(nfID)))
        if 'hevc-dv5-' not in manifest_keys_dv or 'hevc-dv-' not in manifest_keys_dv:
            return KEYS
        else:
