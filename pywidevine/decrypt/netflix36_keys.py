...
if __name__ == 'pywidevine.decrypt.netflix36_keys':
    ...
    def GettingKEYS_Netflix(nfID):
        ...
        KEYS = []
        manifest_keys_main = str(MSL.load_manifest(MSL(int(nfID)), int(nfID)))
        if 'playready-h264mpl' not in manifest_keys_main or 'playready-h264bpl' not in manifest_keys_main:
            return KEYS
        else:
