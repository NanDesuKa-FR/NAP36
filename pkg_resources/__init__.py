...
# Embedded file name: lib\site-packages\setuptools-39.0.1-py3.6.egg\pkg_resources\__init__.py
"""
...
import sys, os, io, time, re, types, zipfile, zipimport, warnings, stat, functools, pkgutil, operator, platform, collections, plistlib, email.parser, errno, tempfile, textwrap, itertools, inspect
...
class ResolutionError(Exception):
    __doc__ = 'Abstract base for dependency resolution errors'
...
class DistributionNotFound(ResolutionError):
    __doc__ = 'A requested distribution was not found'
...
class UnknownExtra(ResolutionError):
    __doc__ = 'Distribution doesn\'t have an "extra feature" of the given name'
...
def get_distribution(dist):
    ...
    if isinstance(dist, six.string_types):
        ...
        raise isinstance(dist, Distribution) or TypeError('Expected string, Requirement, or Distribution', dist)
    return dist
...
class IResourceProvider(IMetadataProvider):
    __doc__ = 'An object that provides access to package resources'
...
class WorkingSet(object):
    __doc__ = 'A collection of active distributions on sys.path (or a similar list)'
                        ...
                        if dist is None or dist not in req and replace_conflicting:
                            ...
                            if env is None:
                                if dist is None:
                                    ...
                                    env = Environment([])
                                    ws = WorkingSet([])
                            dist = best[req.key] = env.best_match(req,
                              ws, installer, replace_conflicting=replace_conflicting)
                            if dist is None:
...
class Environment(object):
    __doc__ = 'Searchable snapshot of distributions on a search path'
...
class ExtractionError(RuntimeError):
    __doc__ = 'An error occurred extracting a resource\n\n    The following attributes are available from instances of this exception:\n\n    manager\n        The resource manager that raised this exception\n\n    cache_path\n        The base directory for resource extraction\n\n    original_error\n        The exception instance that caused extraction to fail\n    '


class ResourceManager:
    __doc__ = 'Manage resource extraction and packages'
    ...
    def _warn_unsafe_extraction_path(path):
        ...
        """
        if os.name == 'nt':
            if not path.startswith(os.environ['windir']):
                return
...
class NullProvider:
    __doc__ = 'Try to implement resources and metadata for arbitrary PEP 302 loaders'
    egg_name = None
    egg_info = None
    loader = None
...
class EggProvider(NullProvider):
    __doc__ = 'Provider based on a virtual filesystem'
...
class DefaultProvider(EggProvider):
    __doc__ = 'Provides access to package resources in the filesystem'
...
class EmptyProvider(NullProvider):
    __doc__ = 'Provider that returns nothing for all requests'
    module_path = None
...
class ZipProvider(EggProvider):
    __doc__ = 'Resource support for zips and eggs'
    eagers = None
...
class FileMetadata(EmptyProvider):
    __doc__ = 'Metadata handler for standalone PKG-INFO files\n\n    Usage::\n\n        metadata = FileMetadata("/path/to/PKG-INFO")\n\n    This provider rejects all data and metadata requests except for PKG-INFO,\n    which is treated as existing, and will be the contents of the file at\n    the provided location.\n    '
...
class EggMetadata(ZipProvider):
    __doc__ = 'Metadata provider for .egg files'
...
def find_on_path(importer, path_item, only=False):
    ...
    if _is_unpacked_egg(path_item):
        yield Distribution.from_filename(path_item,
...
def safe_listdir(path):
    ...
    try:
        ...
        ignorable = e.errno in (errno.ENOTDIR, errno.EACCES, errno.ENOENT) or getattr(e, 'winerror', None) == 267
        if not ignorable:
            raise
...
def distributions_from_metadata(path):
    ...
    yield Distribution.from_location(root,
      entry, metadata, precedence=DEVELOP_DIST)
...
class EntryPoint(object):
    __doc__ = 'Object representing an advertised importable object'
...
class Distribution(object):
    __doc__ = 'Wrap an actual or potential sys.path entry w/metadata'
    PKG_INFO = 'PKG-INFO'
    ...
    def from_location(cls, location, basename, metadata=None, **kw):
        ...
        return cls(location, metadata, project_name=project_name, version=version, py_version=py_version, 
         platform=platform, **kw)._reload_version()
    ...
    def _dep_map(self):
        ...
        """
        try:
            return self._Distribution__dep_map
        except AttributeError:
            self._Distribution__dep_map = self._filter_extras(self._build_dep_map())
    ...
    def check_version_conflict(self):
        ...
        for modname in self._get_metadata('top_level.txt'):
            if not modname not in sys.modules:
                if modname in nsp or modname in _namespace_packages:
                    pass
                else:
                    if modname in ('pkg_resources', 'setuptools', 'site'):
                        ...
                        if fn:
                            if not normalize_path(fn).startswith(loc):
                                if fn.startswith(self.location):
                                    continue
...
class DistInfoDistribution(Distribution):
    ...
    def _dep_map(self):
        try:
            return self._DistInfoDistribution__dep_map
        except AttributeError:
            self._DistInfoDistribution__dep_map = self._compute_dependencies()
            return self._DistInfoDistribution__dep_map

    def _compute_dependencies(self):
        """Recompute this distribution's dependencies."""
        dm = self._DistInfoDistribution__dep_map = {None: []}
        reqs = []
...
class Requirement(packaging.requirements.Requirement):
    ...
    def __init__(self, requirement_string):
        ...
        self._Requirement__hash = hash(self.hashCmp)
    ...
    def __hash__(self):
        return self._Requirement__hash
