...
# Embedded file name: lib\site-packages\google\protobuf\internal\well_known_types.py
...
class Error(Exception):
    __doc__ = 'Top-level module error.'
...
def _CheckDurationValid(seconds, nanos):
    if seconds < -_DURATION_SECONDS_MAX or seconds > _DURATION_SECONDS_MAX:
        raise Error('Duration is not valid: Seconds {0} must be in range [-315576000000, 315576000000].'.format(seconds))
    else:
        if nanos <= -_NANOS_PER_SECOND or nanos >= _NANOS_PER_SECOND:
            raise Error('Duration is not valid: Nanos {0} must be in range [-999999999, 999999999].'.format(nanos))
        if nanos < 0 and seconds > 0 or nanos > 0 and seconds < 0:
            raise Error('Duration is not valid: Sign mismatch.')
...
def _SnakeCaseToCamelCase(path_name):
    ...
    for c in path_name:
        if c.isupper():
            raise Error('Fail to print FieldMask to Json string: Path name {0} must not contain uppercase letters.'.format(path_name))
        if after_underscore:
            if c.islower():
                result.append(c.upper())
                after_underscore = False
            else:
                raise Error('Fail to print FieldMask to Json string: The character after a "_" must be a lowercase letter in path name {0}.'.format(path_name))
        else:
            if c == '_':
    ...
    if after_underscore:
        raise Error('Fail to print FieldMask to Json string: Trailing "_" in path name {0}.'.format(path_name))
    return ''.join(result)
