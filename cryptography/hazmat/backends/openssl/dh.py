...
# Embedded file name: lib\site-packages\cryptography\hazmat\backends\openssl\dh.py
...
def _handle_dh_compute_key_error(errors, backend):
    lib = backend._lib
    backend.openssl_assert(errors[0]._lib_reason_match(lib.ERR_LIB_DH, lib.DH_R_INVALID_PUBKEY))
...
class _DHPrivateKey(object):
    ...
    def private_bytes(self, encoding, format, encryption_algorithm):
        ...
        if not self._backend._lib.Cryptography_HAS_EVP_PKEY_DHX:
            ...
            if q[0] != self._backend._ffi.NULL:
                raise UnsupportedAlgorithm('DH X9.42 serialization is not supported', _Reasons.UNSUPPORTED_SERIALIZATION)
        return self._backend._private_key_bytes(encoding, format, encryption_algorithm, self._evp_pkey, self._dh_cdata)
