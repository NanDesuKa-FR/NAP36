...
# Embedded file name: lib\site-packages\cryptography\hazmat\backends\openssl\rsa.py
    ...
    if not isinstance(padding, AsymmetricPadding):
        ...
        if isinstance(padding, PKCS1v15):
            ...
            if isinstance(padding, OAEP):
                ...
                raise UnsupportedAlgorithm('{0} is not supported by this backend.'.format(padding.name), _Reasons.UNSUPPORTED_PADDING)
...
def _handle_rsa_enc_dec_error(backend, key):
    ...
    if isinstance(key, _RSAPublicKey):
        backend.openssl_assert(errors[0].reason == backend._lib.RSA_R_DATA_TOO_LARGE_FOR_KEY_SIZE)
        raise ValueError('Data too long for key size. Encrypt less data or use a larger key size.')
    else:
        decoding_errors = [
...
def _rsa_sig_setup(backend, padding, algorithm, key, data, init_func):
    ...
    if res == 0:
        ...
        raise UnsupportedAlgorithm('{0} is not supported by this backend for RSA signing.'.format(algorithm.name), _Reasons.UNSUPPORTED_HASH)
    res = backend._lib.EVP_PKEY_CTX_set_rsa_padding(pkey_ctx, padding_enum)
    backend.openssl_assert(res > 0)
    if isinstance(padding, PSS):
...
def _rsa_sig_sign(backend, padding, algorithm, private_key, data):
    ...
    if res != 1:
        ...
        if errors[0].reason == backend._lib.RSA_R_DATA_TOO_LARGE_FOR_KEY_SIZE:
            ...
            backend.openssl_assert(errors[0].reason == backend._lib.RSA_R_DIGEST_TOO_BIG_FOR_RSA_KEY)
...
class _RSAPrivateKey(object):
    ...
    def private_bytes(self, encoding, format, encryption_algorithm):
        return self._backend._private_key_bytes(encoding, format, encryption_algorithm, self._evp_pkey, self._rsa_cdata)
