...
# Embedded file name: lib\site-packages\cryptography\hazmat\backends\openssl\backend.py
from __future__ import absolute_import, division, print_function
import base64, collections, contextlib, itertools
from contextlib import contextmanager
import asn1crypto.core, six
...
from cryptography.hazmat.backends.openssl.decode_asn1 import _CRL_ENTRY_REASON_ENUM_TO_CODE, _Integers
...
class Backend(object):
    ...
    def _evp_md_from_algorithm(self, algorithm):
        if algorithm.name == 'blake2b' or algorithm.name == 'blake2s':
            alg = '{0}{1}'.format(algorithm.name, algorithm.digest_size * 8).encode('ascii')
        else:
    ...
    def register_cipher_adapter(self, cipher_cls, mode_cls, adapter):
        if (cipher_cls, mode_cls) in self._cipher_registry:
            raise ValueError('Duplicate registration for: {0} {1}.'.format(cipher_cls, mode_cls))
    ...
    def create_x509_csr(self, builder, private_key, algorithm):
        if not isinstance(algorithm, hashes.HashAlgorithm):
            ...
            if res == 0:
                ...
                self.openssl_assert(errors[0]._lib_reason_match(self._lib.ERR_LIB_RSA, self._lib.RSA_R_DIGEST_TOO_BIG_FOR_RSA_KEY))
    ...
    def create_x509_certificate(self, builder, private_key, algorithm):
        if not isinstance(builder, x509.CertificateBuilder):
            ...
            self.openssl_assert(res == 1)
            self._set_asn1_time(self._lib.X509_get_notBefore(x509_cert), builder._not_valid_before)
            self._set_asn1_time(self._lib.X509_get_notAfter(x509_cert), builder._not_valid_after)
            self._create_x509_extensions(extensions=(builder._extensions),
              handlers=_EXTENSION_ENCODE_HANDLERS,
              x509_obj=x509_cert,
        ...
        if evp_pkey == self._ffi.NULL:
            if userdata.error != 0:
                ...
                if userdata.error == -1:
                    ...
                    raise ValueError('Passwords longer than {0} bytes are not supported by this backend.'.format(userdata.maxsize - 1))
            else:
                self._handle_key_loading_error()
        evp_pkey = self._ffi.gc(evp_pkey, self._lib.EVP_PKEY_free)
    ...
    def _handle_key_loading_error(self):
        ...
        if not errors:
            ...
            if errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_BAD_DECRYPT) or errors[0]._lib_reason_match(self._lib.ERR_LIB_PKCS12, self._lib.PKCS12_R_PKCS12_CIPHERFINAL_ERROR):
                ...
                if errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_UNKNOWN_PBE_ALGORITHM) or errors[0]._lib_reason_match(self._lib.ERR_LIB_PEM, self._lib.PEM_R_UNSUPPORTED_ENCRYPTION):
    ...
    def elliptic_curve_supported(self, curve):
        ...
        if group == self._ffi.NULL:
            errors = self._consume_errors()
            self.openssl_assert(curve_nid == self._lib.NID_undef or errors[0]._lib_reason_match(self._lib.ERR_LIB_EC, self._lib.EC_R_UNKNOWN_GROUP))
            return False
        else:
    ...
    def generate_elliptic_curve_private_key(self, curve):
        ...
        if self.elliptic_curve_supported(curve):
            ...
            return _EllipticCurvePrivateKey(self, ec_cdata, evp_pkey)
        raise UnsupportedAlgorithm('Backend object does not support {0}.'.format(curve.name), _Reasons.UNSUPPORTED_ELLIPTIC_CURVE)
    ...
    def _create_ocsp_basic_response(self, builder, private_key, algorithm):
        ...
        if res != 1:
            errors = self._consume_errors()
            self.openssl_assert(errors[0]._lib_reason_match(self._lib.ERR_LIB_X509, self._lib.X509_R_KEY_VALUES_MISMATCH))
    ...
    def _elliptic_curve_to_nid(self, curve):
        ...
        if curve_nid == self._lib.NID_undef:
            raise UnsupportedAlgorithm('{0} is not a supported elliptic curve'.format(curve.name), _Reasons.UNSUPPORTED_ELLIPTIC_CURVE)
        return curve_nid
    ...
    def _private_key_bytes(self, encoding, format, encryption_algorithm, evp_pkey, cdata):
        ...
        elif encoding is serialization.Encoding.X962:
            raise ValueError('X9.62 format is only valid for EC public keys')
        elif format is serialization.PrivateFormat.Raw:
        ...
        if encoding is serialization.Encoding.PEM:
            if format is serialization.PrivateFormat.PKCS8:
                ...
                if not format is serialization.PrivateFormat.TraditionalOpenSSL:
                    raise AssertionError
                else:
                    if key_type == self._lib.EVP_PKEY_RSA:
            ...
            if encoding is serialization.Encoding.DER:
                ...
                elif not format is serialization.PrivateFormat.PKCS8:
                    raise AssertionError
    ...
    def _public_key_bytes(self, encoding, format, key, evp_pkey, cdata):
        if not isinstance(encoding, serialization.Encoding):
            ...
            if encoding is serialization.Encoding.Raw:
                raise ValueError('raw encoding is invalid with this key or format')
        if format is serialization.PublicFormat.OpenSSH or encoding is serialization.Encoding.OpenSSH:
            if format is not serialization.PublicFormat.OpenSSH or encoding is not serialization.Encoding.OpenSSH:
            ...
            if format is serialization.PublicFormat.SubjectPublicKeyInfo:
                ...
                if format is serialization.PublicFormat.PKCS1:
                    if not self._lib.EVP_PKEY_id(evp_pkey) == self._lib.EVP_PKEY_RSA:
                        raise AssertionError
                    else:
                        if encoding is serialization.Encoding.PEM:
    ...
    def _openssh_public_key_bytes(self, key):
        if isinstance(key, rsa.RSAPublicKey):
            ...
            return b'ssh-rsa ' + base64.b64encode(ssh._ssh_write_string(b'ssh-rsa') + ssh._ssh_write_mpint(public_numbers.e) + ssh._ssh_write_mpint(public_numbers.n))
        else:
            if isinstance(key, dsa.DSAPublicKey):
                public_numbers = key.public_numbers()
                parameter_numbers = public_numbers.parameter_numbers
                return b'ssh-dss ' + base64.b64encode(ssh._ssh_write_string(b'ssh-dss') + ssh._ssh_write_mpint(parameter_numbers.p) + ssh._ssh_write_mpint(parameter_numbers.q) + ssh._ssh_write_mpint(parameter_numbers.g) + ssh._ssh_write_mpint(public_numbers.y))
            else:
                assert isinstance(key, ec.EllipticCurvePublicKey)
                public_numbers = key.public_numbers()
                try:
                    curve_name = {ec.SECP256R1: b'nistp256', 
                     ec.SECP384R1: b'nistp384', 
                     ec.SECP521R1: b'nistp521'}[type(public_numbers.curve)]
                except KeyError:
                    raise ValueError('Only SECP256R1, SECP384R1, and SECP521R1 curves are supported by the SSH public key format')

            point = key.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)
            return b'ecdsa-sha2-' + curve_name + b' ' + base64.b64encode(ssh._ssh_write_string(b'ecdsa-sha2-' + curve_name) + ssh._ssh_write_string(curve_name) + ssh._ssh_write_string(point))
    ...
    def derive_scrypt(self, key_material, salt, length, n, r, p):
        ...
        if res != 1:
            ...
            if not self._lib.CRYPTOGRAPHY_OPENSSL_LESS_THAN_111:
                self.openssl_assert(errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.ERR_R_MALLOC_FAILURE) or errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP, self._lib.EVP_R_MEMORY_LIMIT_EXCEEDED))
            min_memory = 128 * n * r // 1048576
            raise MemoryError('Not enough memory to derive key. These parameters require {} MB of memory.'.format(min_memory))
        return self._ffi.buffer(buf)[:]
