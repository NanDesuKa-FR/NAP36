...
# Embedded file name: lib\site-packages\cryptography\hazmat\primitives\ciphers\base.py
...
class _AEADCipherContext(object):
    ...
    def _check_limit(self, data_size):
        ...
        if self._bytes_processed > self._ctx._mode._MAX_ENCRYPTED_BYTES:
            raise ValueError('{0} has a maximum encrypted byte limit of {1}'.format(self._ctx._mode.name, self._ctx._mode._MAX_ENCRYPTED_BYTES))
    ...
    def authenticate_additional_data(self, data):
        if self._ctx is None:
            ...
            if self._aad_bytes_processed > self._ctx._mode._MAX_AAD_BYTES:
                raise ValueError('{0} has a maximum AAD byte limit of {1}'.format(self._ctx._mode.name, self._ctx._mode._MAX_AAD_BYTES))
