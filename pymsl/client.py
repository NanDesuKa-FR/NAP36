...
# Embedded file name: lib\site-packages\pymsl\client.py
"""
...
import pymsl.utils
from pymsl.exceptions import KeyExchangeError, LicenseError, ManifestError, UserAuthDataError
DEFAULTS = {'esn':pymsl.utils.generate_esn('NFCDCH-02-'), 
 'drm_system':'widevine', 
 'profiles':[
...
ENDPOINTS = {'manifest':BASE_URL + '/pbo_manifests/^1.0.0/router', 
...
class MslClient(object):
    __doc__ = 'This class holds the functions for MSL API interaction'
    ...
    def __init__(self, user_auth_data, **kwargs):
        ...
        self.msl_session = {'user_auth_data':user_auth_data, 
         ...
         'message_id':kwargs.get('message_id', random.randint(0, 4503599627370496)), 
         ...
         'license_path':None}
        self.header = {'sender':self.msl_session['esn'], 
         'renewable':True, 
         'capabilities':{'languages':self.msl_session['languages'], 
          'compressionalgos':[
           '']}, 
         'messageid':self.msl_session['message_id'], 
         'keyrequestdata':[
          {'scheme':'ASYMMETRIC_WRAPPED', 
           'keydata':{'keypairid':'rsaKeypairId', 
            'mechanism':'JWK_RSA', 
            'publickey':base64.b64encode(self.msl_session['keypair'].publickey().exportKey('DER')).decode('utf8')}}]}
        self.msl_session['session_keys'] = self.parse_handshake(self.perform_key_handshake())
    ...
    def load_manifest(self, viewable_id):
        ...
        manifest_request_data = {'version':2, 
         'url':'/manifest', 
         'id':15429961728572, 
         ...
         'params':{'type':'standard', 
          ...
          'videoOutputInfo':[
           {'type':'DigitalVideoOutputDescriptor', 
            'outputType':'unknown', 
            'supportedHdcpVersions':[],  'isHdcpEngaged':False}], 
          'preferAssistiveAudio':False, 
          'isNonMember':False}}
        manifest_request_data['params'].update(self.msl_session['extra_manifest_params'])
        ...
        raise ManifestError(json.loads(base64.b64decode(resp.json()['errordata']).decode('utf8'))['errormsg'])
    ...
    def get_license(self, challenge, session_id):
        ...
        if not isinstance(challenge, bytes):
            ...
            if not self.msl_session['license_path']:
                raise LicenseError('Manifest must be loaded before license is acquired')
            license_request_data = {'version':2, 
    ...
    def perform_key_handshake(self):
        ...
        """
        header = {'entityauthdata':{'scheme':'NONE', 
          'authdata':{'identity': self.msl_session['esn']}}, 
         'signature':''}
        header['headerdata'] = base64.b64encode(pymsl.utils.dumps(self.header).encode('utf8')).decode('utf8')
        payload = {'signature': ''}
        payload['payload'] = base64.b64encode(pymsl.utils.dumps({'sequencenumber':1, 
         'messageid':self.msl_session['message_id'], 
         'endofmsg':True, 
         'data':''}).encode('utf8')).decode('utf8')
    ...
    def parse_handshake(self, response):
        ...
        if response.get('errordata'):
            raise KeyExchangeError(base64.b64decode(response['errordata']).decode('utf8'))
        headerdata = json.loads(base64.b64decode(response['headerdata']).decode('utf8'))
        mastertoken = headerdata['keyresponsedata']['mastertoken']
        sequence_number = json.loads(base64.b64decode(mastertoken['tokendata']).decode('utf8'))['sequencenumber']
        encrypted_encryption_key = base64.b64decode(headerdata['keyresponsedata']['keydata']['encryptionkey'])
        encrypted_sign_key = base64.b64decode(headerdata['keyresponsedata']['keydata']['hmackey'])
        oaep_cipher = PKCS1_OAEP.new(self.msl_session['keypair'])
        encryption_key_data = json.loads(oaep_cipher.decrypt(encrypted_encryption_key).decode('utf8'))
        encryption_key = pymsl.utils.webcrypto_b64decode(encryption_key_data['k'])
        sign_key_data = json.loads(oaep_cipher.decrypt(encrypted_sign_key).decode('utf8'))
    ...
    def generate_msl_request_data(self, data):
        ...
        enc_header = {'headerdata':base64.b64encode(header_envelope).decode('utf8'), 
         'signature':base64.b64encode(header_signature).decode('utf8'), 
         'mastertoken':self.msl_session['session_keys']['mastertoken']}
        payload = {'messageid':self.msl_session['message_id'], 
         'data':base64.b64encode(pymsl.utils.dumps(data).encode('utf8')).decode('utf8'), 
         'sequencenumber':1, 
         'endofmsg':True}
        payload_envelope = pymsl.utils.msl_encrypt(self.msl_session, pymsl.utils.dumps(payload))
        payload_signature = HMAC.new(self.msl_session['session_keys']['sign_key'], payload_envelope, SHA256).digest()
        payload_chunk = {'payload':base64.b64encode(payload_envelope).decode('utf8'), 
    ...
    def decrypt_msl_payload(self, payload):
        ...
        """
        payloads = re.split(',"signature":"[0-9A-Za-z/+=]+"}', payload.split('}}')[1])
        ...
        for chunk in payload_chunks:
            ...
            cipher = AES.new(self.msl_session['session_keys']['encryption_key'], AES.MODE_CBC, base64.b64decode(json.loads(base64.b64decode(encryption_envelope).decode('utf8'))['iv']))
            plaintext = cipher.decrypt(base64.b64decode(json.loads(base64.b64decode(encryption_envelope).decode('utf8'))['ciphertext']))
