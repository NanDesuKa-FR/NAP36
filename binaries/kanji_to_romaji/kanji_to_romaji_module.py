...
# Embedded file name: Nap36Src\binaries\kanji_to_romaji\kanji_to_romaji_module.py
import os, sys
        ...
        if os.path.splitext(f)[1] == '.json' and 'kanji' in f:
            with open((os.path.join(JP_MAPPINGS_PATH, f)), encoding='utf-8-sig') as (data_file):
                ...
                for k in list(data_file_dict.keys()):
                    if k in kanji_romaji_mapping and data_file_dict[k]['w_type'] != kanji_romaji_mapping[k]['w_type']:
                        if 'other_readings' not in kanji_romaji_mapping[k]:
                            kanji_romaji_mapping[k]['other_readings'] = {}
                        kanji_romaji_mapping[k]['other_readings'][data_file_dict[k]['w_type']] = data_file_dict[k]['romaji']
                    else:
...
def is_hiragana(c):
    ...
    return c not in [hiragana_iter_mark, hiragana_voiced_iter_mark, hirgana_soukon_unicode_char] and hiragana_starting_unicode <= c <= hiragana_ending_unicode
...
def is_katakana(c):
    ...
    return c not in [katakana_iter_mark, katakana_voiced_iter_mark,
     katakana_soukon_unicode_char, katakana_long_vowel_mark] and katakana_starting_unicode <= c <= katakana_ending_unicode
...
def translate_particles(kana_list):
    ...
    def type_changes(p, n):
        if get_char_type(p) is not None:
            if get_char_type(n) is not None:
    ...
    for i in range(1, len(kana_list)):
        ...
        if kana_list[i] == no_hira_char:
            if is_noun(prev_c) and is_noun(next_c) or type_changes(prev_c, next_c) or is_noun(prev_c) and is_last_char:
                kana_list[i] = no_prtcle
        elif kana_list[i] == ha_hira_char:
            if is_noun(prev_c) and isinstance(next_c, KanjiBlock) or type_changes(prev_c, next_c) or particle_imm_follows(prev_c, [e_prtcle, to_prtcle, ni_prtcle, de_prtcle]) or is_noun(prev_c) and is_last_char:
                kana_list[i] = wa_prtcle
        elif kana_list[i] == mo_hira_char:
            if is_noun(prev_c) and isinstance(next_c, KanjiBlock) or type_changes(prev_c, next_c) or particle_imm_follows(prev_c, [ni_prtcle, de_prtcle]) or is_noun(prev_c) and is_last_char:
                kana_list[i] = mo_prtcle
        else:
            if kana_list[i] in [he_hira_char, to_hira_char, ni_hira_char, de_hira_char, ga_hira_char] and is_noun(prev_c) and isinstance(next_c, KanjiBlock) or type_changes(prev_c, next_c) or is_noun(prev_c) and is_last_char:
                if kana_list[i] == he_hira_char:
                    kana_list[i] = e_prtcle
                else:
                    if kana_list[i] == to_hira_char:
                        kana_list[i] = to_prtcle
                    else:
                        if kana_list[i] == ni_hira_char:
                            kana_list[i] = ni_prtcle
                        else:
                            if kana_list[i] == de_hira_char:
...
def get_type_if_verb_stem(curr_chars):
    ...
    if 'verb stem' in UnicodeRomajiMapping.kanji_mapping[curr_chars]['w_type']:
        ...
        if 'other_readings' in UnicodeRomajiMapping.kanji_mapping[curr_chars]:
...
def check_for_verb_stem_ending(kana_list, curr_chars, start_pos, char_len):
    ...
    if 'verb stem' in UnicodeRomajiMapping.kanji_mapping[curr_chars]['w_type']:
        ...
        if 'other_readings' in UnicodeRomajiMapping.kanji_mapping[curr_chars]:
            if 'godan verb stem' in UnicodeRomajiMapping.kanji_mapping[curr_chars]['other_readings']:
                dict_entry = {'romaji': UnicodeRomajiMapping.kanji_mapping[curr_chars]['other_readings']['godan verb stem']}
            elif 'ichidan verb stem' in UnicodeRomajiMapping.kanji_mapping[curr_chars]['other_readings']:
                dict_entry = {'romaji': UnicodeRomajiMapping.kanji_mapping[curr_chars]['other_readings']['ichidan verb stem']}
    e_k = None
    e_r = None
...
def has_non_verb_stem_reading(curr_chars):
    ...
    if 'verb stem' not in UnicodeRomajiMapping.kanji_mapping[curr_chars]['w_type']:
        ...
        if 'other_readings' in UnicodeRomajiMapping.kanji_mapping[curr_chars]:
            if any(['verb stem' not in ork for ork in list(UnicodeRomajiMapping.kanji_mapping[curr_chars]['other_readings'].keys())]):
                res = True
    return res
...
def get_verb_stem_romaji(verb_stem_kanji):
    ...
    if 'verb stem' in UnicodeRomajiMapping.kanji_mapping[verb_stem_kanji]['w_type']:
        ...
        if 'other_readings' in UnicodeRomajiMapping.kanji_mapping[verb_stem_kanji]:
            ...
            if curr_chars in UnicodeRomajiMapping.kanji_mapping:
                ...
                if verb_stem_type is not None:
                    ...
                    if ending_kana is not None and ending_romaji is not None:
                        ...
                        conjugated_val = {'romaji':get_verb_stem_romaji(curr_chars) + ending_romaji, 
                         'w_type':'conjugated ' + verb_stem_type}
                        for i in range(start_pos + char_len - 1 + len(ending_kana), start_pos - 1, -1):
                            del kana_list[i]

                        kana_list.insert(start_pos, KanjiBlock(curr_chars + ending_kana, conjugated_val))
                if ending_match_found is False:
                    if has_non_verb_stem_reading(curr_chars):
                        for i in range(start_pos + char_len - 1, start_pos - 1, -1):
                            del kana_list[i]

                        kana_list.insert(start_pos, KanjiBlock(curr_chars, UnicodeRomajiMapping.kanji_mapping[curr_chars]))
            char_len -= 1
...
def translate_soukon(partial_kana):
    ...
    for c in reversed(partial_kana):
        if c == hirgana_soukon_unicode_char or c == katakana_soukon_unicode_char:
...
def translate_kana_iteration_mark(kana):
    ...
    for c in kana:
        if c == hiragana_iter_mark or c == katakana_iter_mark:
            ...
            if c == hiragana_voiced_iter_mark or c == katakana_voiced_iter_mark:
