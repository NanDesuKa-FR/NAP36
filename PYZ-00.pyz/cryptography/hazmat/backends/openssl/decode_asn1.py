...
# Embedded file name: lib\site-packages\cryptography\hazmat\backends\openssl\decode_asn1.py
from __future__ import absolute_import, division, print_function
import datetime, ipaddress, asn1crypto.core, six
...
def _decode_general_name(backend, gn):
    ...
    raise x509.UnsupportedGeneralNameType('{0} is not a supported type'.format(x509._GENERAL_NAMES.get(gn.type, gn.type)), gn.type)
...
class _X509ExtensionParser(object):
    ...
    def parse(self, backend, x509_obj):
        ...
        for i in range(self.ext_count(backend, x509_obj)):
            ...
            if oid in seen_oids:
                raise x509.DuplicateExtension('Duplicate {0} extension found'.format(oid), oid)
            if oid == ExtensionOID.TLS_FEATURE:
                data = backend._lib.X509_EXTENSION_get_data(ext)
                parsed = _Integers.load(_asn1_string_to_bytes(backend, data))
                value = x509.TLSFeature([_TLS_FEATURE_TYPE_TO_ENUM[x.native] for x in parsed])
                ...
                if oid == ExtensionOID.PRECERT_POISON:
                    data = backend._lib.X509_EXTENSION_get_data(ext)
                    parsed = asn1crypto.core.Null.load(_asn1_string_to_bytes(backend, data))
                ...
                try:
                    ...
                    if ext_data == backend._ffi.NULL:
                        ...
                        raise ValueError("The {0} extension is invalid and can't be parsed".format(oid))
...
def _asn1_string_to_utf8(backend, asn1_string):
    ...
    if res == -1:
        raise ValueError('Unsupported ASN1 string type. Type: {0}'.format(asn1_string.type))
    backend.openssl_assert(buf[0] != backend._ffi.NULL)
    buf = backend._ffi.gc(buf, lambda buffer: backend._lib.OPENSSL_free(buffer[0]))
