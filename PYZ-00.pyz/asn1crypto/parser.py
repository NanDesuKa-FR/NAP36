...
# Embedded file name: lib\site-packages\asn1crypto\parser.py
"""
    ...
    if not isinstance(class_, int):
        ...
        if class_ < 0 or class_ > 3:
            ...
            if not isinstance(method, int):
                ...
                raise isinstance(tag, int) or TypeError('tag must be an integer, not %s' % type_name(tag))
            if tag < 0:
                raise ValueError('tag must be greater than zero, not %s' % tag)
        raise isinstance(contents, byte_cls) or TypeError('contents must be a byte string, not %s' % type_name(contents))
...
def _parse(encoded_data, data_len, pointer=0, lengths_only=False):
    ...
    if data_len < pointer + 2:
        ...
        if length_octet >> 7 == 0:
            if lengths_only:
                return (
                 pointer, pointer + (length_octet & 127))
...
def _dump_header(class_, method, tag, contents):
    ...
    if tag >= 31:
        ...
        while tag > 0:
            continuation_bit = 128 if tag > 127 else 0
            header += chr_cls(continuation_bit | tag & 127)
            tag = tag >> 7
    ...
    if length <= 127:
        ...
        header += chr_cls(128 | len(length_bytes))
        header += length_bytes
    return header