...
# Embedded file name: lib\site-packages\cffi\api.py
import sys, types
...
class FFI(object):
    ...
    def typeof(self, cdecl):
        ...
        elif isinstance(cdecl, self.CData):
    ...
    def getctype(self, cdecl, replace_with=''):
        ...
        if isinstance(cdecl, basestring):
            ...
            if replace_with.startswith('*'):
                if '&[' in self._backend.getcname(cdecl, '&'):
 ...
 source, tmpdir=tmpdir, extradir=tmpdir, source_extension=source_extension, 
...
def _make_ffi_library(ffi, libname, flags):
    ...
    def update_accessors():
        ...
        for key, (tp, _) in ffi._parser._declarations.items():
            tag, name = isinstance(tp, model.EnumType) or key.split(' ', 1)
            if tag == 'function':
