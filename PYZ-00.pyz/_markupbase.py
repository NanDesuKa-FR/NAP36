...
class ParserBase:
                        ...
                        if c in self._decl_otherchars:
                            ...
                            if c == '[':
                                if decltype == 'doctype':
                                    ...
                                    if decltype in frozenset({'attlist', 'link', 'linktype', 'element'}):
    ...
    def parse_marked_section(self, i, report=1):
        ...
        if not rawdata[i:i + 3] == '<![':
            ...
            if j < 0:
                return j
            if sectName in frozenset({'temp', 'rcdata', 'cdata', 'ignore', 'include'}):
                match = _markedsectionclose.search(rawdata, i + 3)
            else:
                if sectName in frozenset({'if', 'endif', 'else'}):
            ...
            if c == '<':
                ...
                if rawdata[j:j + 4] == '<!--':
                    ...
                    if j < 0:
                        ...
                        if j == -1:
                            return -1
                        if name not in frozenset({'attlist', 'entity', 'element', 'notation'}):
