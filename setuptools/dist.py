...
# Embedded file name: lib\site-packages\setuptools-39.0.1-py3.6.egg\setuptools\dist.py
__all__ = ['Distribution']
import re, os, warnings, numbers, distutils.log, distutils.core, distutils.cmd, distutils.dist, itertools
...
def get_metadata_version(dist_md):
    if dist_md.long_description_content_type or dist_md.provides_extras:
        ...
        if dist_md.maintainer is not None or dist_md.maintainer_email is not None or getattr(dist_md, 'python_requires', None) is not None:
            return StrictVersion('1.2')
        if dist_md.provides or dist_md.requires or dist_md.obsoletes or dist_md.classifiers or dist_md.download_url:
...
class Distribution(Distribution_parse_config_files, _Distribution):
    ...
    def __init__(self, attrs=None):
        ...
        self.metadata.project_urls = getattr(self.metadata, 'project_urls', self.project_urls)
        self.metadata.long_description_content_type = attrs.get('long_description_content_type')
        self.metadata.provides_extras = getattr(self.metadata, 'provides_extras', set())
